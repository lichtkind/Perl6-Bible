...
[Update: For instance, despite the beautiful lyrics above, diamond
does not live, tilde is now the concatenate operator, and star
as a prefix operator has mutated into the C<[,]> reduce operator.
(Though C<*> in a signature still means "slurpy".)]
...
[Update: Optional args are now marked with a C<?> suffix or a default
assignment.]
...
[Update: iterating a filehandle is now C<@$fh> or C<=$fh>.]
...
[Update: C<qw()> would now be a function call.  In general you'd use C<< <...> >> instead.]
...
[Update a constant hash subscript would now be C<< .<fh> >> instead.
The C<irs> property is now C<newline>.]
...
[Update: instead of underline, C<< prefix:<~> >> is now the string
context operator.]
...
[Update: Since C<~> is concatenation, numeric XOR is now C<+^> instead.]
...
[Update: These closure arguments now require a comma after them.
(And a single-arg sort routine will do the Schwartzian Transform for
you automatically, but you can still do it this way.)]
