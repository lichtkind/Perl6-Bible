...
[Update: If the inside character is an angle bracket, then a corresponding
number of angles must close the construct.]
...
[Update: Follows ordinary SMD rules, so grammar rules take precedence over lexical rules, which aren't seen unless SMD fails over to MMD.]
...
will be mistaken for C<< <<...>> >>, which doesn't work too well.]
...
[Update: Actually, you can turn C<:i> off with C<:!i> now.]
...
[Update: C<$()> is gone for that meaning, so you have to say
C<< <{lc $foo}> >> or some such.]
...
[Update: There's now an C<< <at($oldpos)> >> assertion for that.  And
the assertion syntax changed from C<< <(...)> >> to C<< <?{...}> >>.]
...
adjacent to the final option.  They may be separated with whitespace.]
...
stay with common culture.  Since the early meaning of C<$()> is also
gone, just interpolate code using a closure: S<C<s/pat/{ code }/>>.]
...
[Update: The modifier is currently C<:sigspace>, or C<:s> for short. The
optional whitespace is actually matched by the C<< <?ws> >> rule, which you
can redefine to change the semantics of your significant spaces.]
...
[Update: Now spelled out C<:Perl5> or C<:P5>.]
...
[Update: "rule" is now split into "regex", "token", and "rule".  See S05.]
...
[Update: A code assertion is now spelled C<< <?{ $1 < 582 }> >>.  The
...
[Update: Character class ranges are now C<< <[a..z]> >>.]
...
[Update: C<< <alpha> >> is no longer allowed as the first thing.
Use C<+alpha> instead.]
...
[Update: Only rules of the form C<< <ident> >> or C<< <$ident> >> are
captured by default.  You can use C<:=> to force capture of anything
else, or the C<:keepall> adverb to capture everything else.]
...
C<$/> variable that represents the match state object, and a C<$()>
variable that represents the capture of the entire pattern.  In other
words, C<$()> is exactly like a normal paren capture if you assume
parens around the entire pattern.  You can also explicitly specify
the returned scalar value in any of several ways.]
...
[Update: These days that's C<$/{'somerule'}[0]> or C<< $<somerule><0> >>.]
...
In addition, there can be more than one hash, as in C<< <%a|%b|%c> >>,
which are all treated as one for the longest-token rule.  See S05.]
...
[Update: This is also bogus.  The keys are treated as a set of tokens with
longest-first semantics all keys involved, mapping to arbitrary rules.  See S05.]
...
[Update: Nowadays it's the C<Regex> class, or some derivative.]
...
[Update: These are now handled as macro-like syntactic categories.  See S05.]
...
[Update: C<$()> now represents the entire matched string, but the
...
[Update: That would be C<+m:g/foo/> these days.]
...
[Update: C<dim(9,9,9)> is now spelled C<[^9,^9,^9]>.  See S09.]
