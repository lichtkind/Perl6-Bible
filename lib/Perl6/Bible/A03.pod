...
[Update: We currently have a C<< prefix:<=> >> that iterates an iterator.]
...
[Update: We now distinguish postfix operators from infix by the presence
...
The RFC's proposal concerning C<attributes::get(@a)> stuff is superseded
...
[Update: Turns out that all lists are lazy by default.  Use the C<eager> listop
...
backtracking covers most of this functionality.  See A05 and S05.]
...
[Update: Unary dot turns out to a method call on the current topic.  See A04
and S04.]
...
[Update: List flattening in an rvalue is now done with the C<[,]>
reduction operator.  (The splat star in a signature is unaffected.)]
...
[Update: This is now C<??!!> instead, to avoid visual confusion of double
colons with package names, and to suggest that the "else" part is what is
done if the condition is NOT true.]
...
[Update: Semicolon no longer makes a list of lists, but a multidimensional
slice, each dimension of which is considered a separate pipeline of values.
Whether this is interpreted multidimensionally is up to the binding function.
See S06.]
...
categories as well, such as C<postfix:> and C<circumfix:>.]
...
Still, we managed to reduce it from 24 to 22 precedence levels.  See S03.]
