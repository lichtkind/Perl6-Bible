...
storage you now use "C<!>" in place of the "C<.>".  Within the lexical
scope of the class you may omit even that.]
...
metaobjects use packages as their global storage.  That is, the
metaclass "does" the Package interface, on some level or other, if
it wants to make global names visible through the package interface.]
...
variable without first declaring it as tieable, or (preferably) tying
...
[Update: The C<$()> notation means something else now.  Use
...
[Update: These days the C<*> would be replaced by a C<[,]> reduction.]
...
[Update: We now do "last wins" semantics for named args, unless the
argument in question is declared as an array, in which case it gets
all the arguments of that name.]
...
[Update: But this one actually I<is> the new self, the candidate created
by C<CREATE> above.  It's just not entirely instantiated until C<BUILDALL> is
done.]
...
[Update: C<@!legs> now.  The C<!> may not be omitted in this case.]
...
[Update: It is unlikely that there will be a C<.dispatcher> method.  This
is all delegated to the C<.meta> object now, at least in the abstract.]
...
[Update: Those autoloaders are now called C<AUTODEF> and C<CANDO>.]
...
[Update: The actual, most-unique "long name" is really till the last colon,
not the first.  But the tiebreaking still happens as described above.]
...
[Update: Multi submethods are visible to both the single dispatch and
multiple dispatcher.  Multi methods are visible only within the constraints
of single dispatch.]
...
[Update: Instead of C<::_> we have C<::?CLASS> to mean the generic
class.]
...
[Update: Just as there's no C<Class> type anymore, there's also no
C<Role> type.  The metaobject handles everything type related (except
the name, which is a package name just like any other type).]
...
[Update: That should read "put together your C<MetaClass> object (and
the package associated with the name, if any)".]
...
[Update: That'd have to be self.Sentry etc. now.]
...
[Update: An infinite range is now written C<(1.2.1..*)>.
...
evaluates the right side in list context.

[Update: That is now false.  A subscripted lvalue expression is always
assumed to be scalar unless it is in parentheses.)

As with all other uses of the French quotes in Perl 6, you can always use:

    %hash<<alice>> = @x;

if you can't figure out how to type C<< ^K < < >> or C<< ^K > > >> in vim.
